...
IS_EXIST = os.path.exists(DATA_RESULT_3D)
if not IS_EXIST:
    os.makedirs(DATA_RESULT_3D)

# folder management: global 3D_COUNT
DATA_RESULT_3D_COUNT = f"{DATA_RESULT_3D}/COUNT"
IS_EXIST = os.path.exists(DATA_RESULT_3D_COUNT)
if not IS_EXIST:
    os.makedirs(DATA_RESULT_3D_COUNT)

# folder management: per PID range
path_table_3d_count = f"{DATA_RESULT_3D_COUNT}/{args.pid_inf}_{args.pid_sup}_{PSEUDO_COUNTER}"
IS_EXIST = os.path.exists(path_table_3d_count)
if not IS_EXIST:
